# G3 Main Results — Cross-Architecture Configs

**Status:** NOT CI-verified — run on the remote 2×A100 box.

This document walks through the complete run order for all five datasets (AG News, SST-2, IMDB, Yahoo, HuffPost) used to fill the main-results table with a 2-encoder / 2-decoder roster.
...
```bash
uv run promptillery materialize-sft examples/paper/G3_<D>_materialize.yaml \
...
```bash
uv run promptillery ablation examples/paper/G3_<D>_ettin_encoder.yaml --no-cleanup
uv run promptillery ablation examples/paper/G3_<D>_ettin_decoder.yaml --no-cleanup
uv run promptillery ablation examples/paper/G3_<D>_gemma3_270m.yaml  --no-cleanup
```
...
```bash
# Step 1: Materialize gold SFT + canonical labels
uv run promptillery materialize-sft examples/paper/G3_agnews_materialize.yaml \
...
uv run promptillery ablation examples/paper/G3_agnews_ettin_encoder.yaml --no-cleanup
uv run promptillery ablation examples/paper/G3_agnews_ettin_decoder.yaml --no-cleanup
uv run promptillery ablation examples/paper/G3_agnews_gemma3_270m.yaml  --no-cleanup
...
# Step 1: Materialize gold SFT + canonical labels
uv run promptillery materialize-sft examples/paper/G3_yahoo_materialize.yaml \
...
# Step 2: Run the three students (each as a 1/5/10 ablation)
uv run promptillery ablation examples/paper/G3_yahoo_ettin_encoder.yaml --no-cleanup
uv run promptillery ablation examples/paper/G3_yahoo_ettin_decoder.yaml --no-cleanup
uv run promptillery ablation examples/paper/G3_yahoo_gemma3_270m.yaml  --no-cleanup
...
- **Cycles 1/5/10:** Each ablation run independently trains for 1, 5, or 10 cycles. The three `heldout_test` numbers become the three cycle rows in the main-results table.
