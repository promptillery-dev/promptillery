...
### Causal LM SFT / SLM (`student_type: "causal_lm_sft"` or `"slm"`)

`"slm"` is an alias for `"causal_lm_sft"` — the causal decoder student. Training
runs on TRL's `SFTTrainer`, and by default uses **completion-only loss**: the
...
Profile a trained student's inference **latency** (p50/p95) and **throughput** on one fixed device, and report the **teacher's cost per 1K calls** (reused from the run's token tracking). Writes a cacheable `profile.json` next to the checkpoint; downstream experiments read it into the deployment table and can reuse a saved profile (e.g. a large student's) rather than re-running it.

The student side is **latency/throughput only** — no derived dollars, since a locally-run student has no API price. Profiling is architecture-agnostic: it times `generate()` for decoder/SLM students, a forward pass for encoder classifiers, and `predict()` for FastText.

```bash
promptillery profile <config_file> [--model-path <path>] [--split <split>] [--device <device>] [--iterations <n>] [--warmup <n>] [--teacher-calls <n>] [--base-dir <directory>]
```
...
- `--model-path`, `-m`: Path to trained model checkpoint. If not provided, auto-detects the most recent model matching the experiment name.
- `--split`, `-s`: Dataset split to profile on (default: "test")
- `--device`: Device to profile on — `cpu`, `cuda`, or `cuda:N` (default: `cuda` if available, else `cpu`). The device — plus GPU name, CPU, and torch version — is stamped into `profile.json` for reproducibility on "one fixed CPU+GPU pair".
- `--iterations`, `-n`: Number of measured single-request calls (default: 50)
