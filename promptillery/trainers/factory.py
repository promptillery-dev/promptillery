...
# Student types that train a causal decoder on SFT-style prompt/response records.
# Engine paths (online augmentation, teacher-token accounting) branch on these,
# so both the original name and the 'slm' alias must be recognised together.
SFT_STUDENT_TYPES = frozenset({"causal_lm_sft", "slm"})
