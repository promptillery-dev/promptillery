...
def _fasttext_line(text: Any) -> str:
    """Collapse all whitespace (including newlines) to single spaces.

    FastText is line-oriented: a newline is a document boundary, and
    ``model.predict`` raises on any input containing one. Real inputs (e.g.
    Banking77 messages) can contain newlines, so we normalize to a single line
    before both training and prediction. This preserves the tokens fasttext
    sees; it only removes the line breaks it cannot handle.
    """
...
class FastTextTrainer(BaseTrainer):
    ...
    def train(self) -> fasttext.FastText:
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
            for i in range(len(train_data)):
                text = _fasttext_line(train_data[i][text_field])
    ...
    def evaluate(self, model: fasttext.FastText, split: str = "test") -> Dict[str, Any]:
        ...
        texts = [_fasttext_line(test_data[i][text_field]) for i in range(len(test_data))]
        ...
        fidelity = self._teacher_fidelity_from_label_ids(pred_labels, split)
        if fidelity is not None:
            scores["teacher_fidelity"] = fidelity
    ...
    def predict_for_augmentation(
        ...
        texts = [_fasttext_line(train_data[i][text_field]) for i in range(len(train_data))]
    ...
    def get_detailed_predictions(
        ...
        texts = [_fasttext_line(data[i][text_field]) for i in range(len(data))]
