...
from ..utils import extract_final_number
from .base import BaseTrainer, PredictionResult

logger = logging.getLogger(__name__)


def kd_loss(student_logits, teacher_logits, labels, *, temperature=1.0, ce_weight=0.0):
    """Token-level forward KL(teacher || student) on completion tokens.

    Logits are unshifted [batch, seq, vocab]; labels carry -100 on prompt/pad
    positions (the completion_only_loss mask). Mixed with CE by ce_weight:
    (1 - ce_weight) * KL * T^2 + ce_weight * CE.
    """
    ...
    log_p_student = F.log_softmax(student / temperature, dim=-1)
    log_p_teacher = F.log_softmax(teacher / temperature, dim=-1)
    kl_per_token = F.kl_div(
        log_p_student, log_p_teacher, log_target=True, reduction="none"
    ).sum(-1)
    kd = (kl_per_token * mask).sum() / denom * (temperature**2)
    if ce_weight <= 0.0:
        return kd
    ce = F.cross_entropy(
        student.reshape(-1, student.size(-1)), target.reshape(-1), ignore_index=-100
    )
    return (1.0 - ce_weight) * kd + ce_weight * ce


def _assert_kd_tokenizer_match(student_tokenizer, teacher_tokenizer):
    """Token-level KL is undefined across tokenizers: hard-fail on mismatch."""
    if student_tokenizer.get_vocab() != teacher_tokenizer.get_vocab():
        raise ValueError(
            "kd.teacher_model tokenizer vocab differs from the student tokenizer; "
            "token-level logit KD requires an identical tokenizer."
        )


def _assert_kd_vocab_size_match(student_model, teacher_model):
    """Logit KD needs equal logit dims; padded lm_head vocab_size can differ
    even when tokenizers match."""
    s, t = student_model.config.vocab_size, teacher_model.config.vocab_size
    if s != t:
        raise ValueError(
            f"kd.teacher_model config.vocab_size ({t}) differs from the student's "
            f"({s}); token-level logit KD requires identical logit dimensions."
        )
...
class CausalLMSFTTrainer(BaseTrainer):
    ...
    def __init__(self, config, dataset, out_dir):
        ...
        self.gold_answer_field = self.trainer_config.get(
    ...
    def _completion_only_loss_enabled(self) -> bool:
        """Whether the loss is computed on response tokens only (prompt masked)."""
        return bool(self.trainer_config.get("completion_only_loss", True))

    def _build_data_collator(self, tokenizer: Any = None):
        """Return the training collator for the current masking configuration.

        With completion-only loss the labels are pre-masked in ``prepare_data``,
        so a padding collator that preserves precomputed labels is used. The
        plain causal-LM collator (which derives labels from ``input_ids``) is
        kept for the full-sequence-loss path.
        """
        tokenizer = tokenizer or self.tokenizer
        if self._completion_only_loss_enabled():
    ...
    def _build_prompt_prefixes(
        self, examples: Dict[str, List[Any]]
    ) -> List[str] | None:
        """Return per-example prompt prefixes for completion-only masking.

        Returns ``None`` when the batch has no prompt/response boundary (e.g. a
        preformatted text field), in which case the whole sequence is trained on.
        The prefix mirrors ``_format_generation_prompt`` so the masked training
        boundary matches the inference boundary.
        """
        has_prompt_response = (
            self.prompt_field in examples and self.response_field in examples
        )
        uses_prompt_response = has_prompt_response and (
            not self.explicit_text_field or self.text_field == self.prompt_field
        )
        if not uses_prompt_response and self.text_field in examples:
            return None
        if has_prompt_response:
            return [
                self._format_generation_prompt(str(prompt))
                for prompt in examples[self.prompt_field]
            ]
        if "instruction" in examples and "output" in examples:
            inputs = examples.get("input") or [""] * len(examples["instruction"])
            return [
                self._format_generation_prompt(
                    "\n".join(part for part in [str(instruction), str(input_)] if part)
                )
                for instruction, input_ in zip(examples["instruction"], inputs)
            ]
        return None

    @staticmethod
    def _completion_boundary(full_ids: List[int], prefix_ids: List[int]) -> int:
        """Length of the leading run of ``full_ids`` shared with the prompt prefix.

        Uses the longest common prefix so trailing special tokens on the prefix
        (e.g. a BERT ``[SEP]``) do not misalign the boundary. Never masks the
        entire sequence: at least one response token stays as a target.
        """
        boundary = 0
        ...
        if not full_ids:
            return 0
        return min(boundary, len(full_ids) - 1)

    def prepare_data(self, split: str) -> Dataset:
        """Tokenize SFT data for causal language modeling."""
        ds = self.dataset[split]
        mask_completion = self._completion_only_loss_enabled()
        warned_full_sequence = False
        ...
        def tokenize(examples: Dict[str, List[Any]]) -> Dict[str, Any]:
            ...
            if not mask_completion:
                return encoded

            prefixes = self._build_prompt_prefixes(examples)
            if prefixes is None:
                # No prompt/response boundary: train on the whole sequence.
                nonlocal warned_full_sequence
                if not warned_full_sequence:
                    logger.info(
                        "completion_only_loss is enabled but split %s has no "
                        "prompt/response boundary; training on full sequences",
                        split,
                    )
                    warned_full_sequence = True
                encoded["labels"] = [list(ids) for ids in encoded["input_ids"]]
                return encoded

            nonlocal masked_token_count, label_token_count
            labels: List[List[int]] = []
            for prefix, ids in zip(prefixes, encoded["input_ids"]):
                prefix_ids = self.tokenizer(
                    prefix,
                    truncation=True,
                    max_length=self.max_seq_length,
                )["input_ids"]
                boundary = self._completion_boundary(ids, prefix_ids)
                labels.append([-100] * boundary + list(ids[boundary:]))
                masked_token_count += boundary
                label_token_count += len(ids)
        ...
        if mask_completion and label_token_count:
            ...
            logger.info(
                ...
                masked_token_count,
                label_token_count,
                100.0 * masked_token_count / label_token_count,
                split,
            )
        return mapped
    ...
    def _kd_config(self) -> dict:
        config = self.trainer_config.get("kd") or {}
        return config if config.get("enabled") else {}

    def _load_kd_teacher(self, kd_config):
        """Co-located (or pinned, via teacher_device_map) frozen teacher for
        on-the-fly full-vocab logit KD."""
        dtype = getattr(torch, str(kd_config.get("teacher_dtype", "bfloat16")))
        device_map = kd_config.get("teacher_device_map")
        extra_kwargs = {"device_map": device_map} if device_map else {}
        ...
        teacher_tokenizer = AutoTokenizer.from_pretrained(
            kd_config["teacher_model"], trust_remote_code=self.trust_remote_code
        )
        _assert_kd_tokenizer_match(self.tokenizer, teacher_tokenizer)
        _assert_kd_vocab_size_match(self.model, teacher)
        return teacher

    def _build_sft_trainer(
        self, train_dataset: Dataset, eval_dataset: Dataset | None, has_eval: bool
    ):
        """Construct the TRL SFTTrainer over a pre-tokenized SFT dataset.

        The dataset is pre-tokenized in ``prepare_data`` and, for completion-only
        loss, already carries prompt-masked ``labels``. ``skip_prepare_dataset``
        tells SFTTrainer to consume it verbatim (no re-tokenizing/packing), and
        ``completion_only_loss=False`` disables TRL's own masking since we supply
        the masked labels and collator ourselves.
        """
        from trl import SFTConfig, SFTTrainer

        trainer_cls = SFTTrainer
        kd_config = self._kd_config()
        if kd_config:
            teacher = self._load_kd_teacher(kd_config)
            temperature = float(kd_config.get("temperature", 1.0))
            ce_weight = float(kd_config.get("ce_weight", 0.0))
            teacher_device_map = kd_config.get("teacher_device_map")

            class KDSFTTrainer(SFTTrainer):
                def compute_loss(
                    self, model, inputs, return_outputs=False, num_items_in_batch=None
                ):
                    # num_items_in_batch: transformers-API compatibility only,
                    # intentionally unused — kd_loss normalizes per completion
                    # token; revisit if gradient_accumulation_steps > 1 is ever
                    # configured.
                    outputs = model(
                    ...
                    if not teacher_device_map:
                        # No pinned placement: lazily co-locate the teacher
                        # with the student, as before.
                        if teacher.device != device:
                            teacher.to(device)
                        teacher_input_ids = inputs["input_ids"]
                        teacher_attention_mask = inputs.get("attention_mask")
                    else:
                        # Pinned placement (e.g. a 32B teacher on its own
                        # GPU): leave the teacher where it was loaded and
                        # ferry only the batch tensors.
        ...
        args = SFTConfig(
            ...
            # SFTConfig defaults bf16 on (unlike TrainingArguments), which
            # crashes on non-bf16 hardware and silently changes precision
            # elsewhere; keep fp32 unless the config opts in.
            bf16=bool(self.trainer_config.get("bf16", False)),
            fp16=bool(self.trainer_config.get("fp16", False)),
            packing=False,
    ...
    def _normalize_answer(self, value: str) -> str:
        ...
        if mode == "number":
            extracted = extract_final_number(value)
            return extracted if extracted is not None else value.lower()
        if mode == "choice":
    ...
    def _canonical_labels(self) -> List[str]:
        ...
        expected_classes = getattr(self.cfg, "num_classes", None) or 0
        if labels and len(labels) < expected_classes:
            logger.warning(
    ...
    def _limited_generation_dataset(
        ...
        label_order = {
            label: position for position, label in enumerate(canonical_labels)
        }
    ...
    def _score_canonical_label_records(
        ...
        score_batch_size = int(
            self.trainer_config.get("canonical_label_score_batch_size", 24)
        )
        ...
        for row_position, (index, row) in enumerate(zip(original_indices, ds)):
            ...
            for label_index, label in enumerate(canonical_labels):
                pairs.append((row_position, prompt + label, label_index))

        logger.info(
            "Scoring %d %s examples against %d canonical labels in batches of %d",
            len(row_infos),
            split,
        ...
        try:
            ...
            for start in range(0, len(pairs), score_batch_size):
                batch_pairs = pairs[start : start + score_batch_size]
                encoded = self.tokenizer(
                    [text for _, text, _ in batch_pairs],
                    return_tensors="pt",
                    padding=True,
                    truncation=True,
                    max_length=self.max_seq_length,
                ...
                prompt_counts = torch.tensor(
                    [
                        max(0, row_infos[row_position]["prompt_token_count"] - 1)
                        for row_position, _, _ in batch_pairs
                    ],
                    device=shift_labels.device,
                ).unsqueeze(1)
                label_mask = positions >= prompt_counts
                ...
                for offset, (row_position, _, label_index) in enumerate(batch_pairs):
                    info = row_infos[row_position]
                    info["label_losses"].append(
                        (label_index, float(losses[offset].cpu()))
                    )
                    info["label_token_counts"].append(
                        (label_index, int(counts[offset].cpu()))
                    )
        finally:
        ...
        for info in row_infos:
            label_losses = [
                loss
                for _, loss in sorted(info["label_losses"], key=lambda item: item[0])
            ]
            label_token_counts = [
                count
                for _, count in sorted(
                    info["label_token_counts"], key=lambda item: item[0]
                )
            ]
    ...
    def _generate_eval_records(
        ...
        if (
            ...
            and self.trainer_config.get("canonical_label_decoding", "score") == "score"
        ):
    ...
    def _generation_summary(self, records: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        normalized_preds = [str(record["normalized_prediction"]) for record in records]
    ...
    def _evaluate_generation(self, trainer: Trainer, split: str) -> Dict[str, Any]:
        ...
        sample_limit = self._generation_sample_limit(detailed=False)
        fidelity_cfg = self.trainer_config.get("fidelity") or {}
        # The fidelity split is the final held-out reporting pass: score the full
        # split so its coverage (and the fidelity denominator) matches the
        # uncapped encoder/FastText path, instead of being truncated by the
        # per-cycle generation cap (which would make a perfect decoder score
        # far below the encoders on an otherwise identical test split).
        if fidelity_cfg.get("teacher_labels_path") and split == fidelity_cfg.get(
            "split", "test"
        ):
            sample_limit = None
        ...
        fidelity = self._teacher_fidelity(records, split)
        if fidelity is not None:
            result["teacher_fidelity"] = fidelity
        return result

    def _teacher_fidelity(
        self, records: List[Dict[str, Any]], split: str
    ) -> float | None:
        """Top-1 student-vs-teacher agreement on `split`, or None if unconfigured.

        Fidelity is computed outside the cached generation summary because it
        depends on the configured teacher labels rather than the generation
        itself. Each record's normalized prediction is joined to the teacher
        label sharing its row index; the teacher labels define the denominator,
        so a row the student never predicted counts as a disagreement.
        """
        fidelity_cfg = self.trainer_config.get("fidelity") or {}
        labels_path = fidelity_cfg.get("teacher_labels_path")
        if not labels_path or not records:
            return None
        if split != fidelity_cfg.get("split", "test"):
            return None

        teacher_labels = load_teacher_eval_labels(labels_path)
        student_by_index = {
            int(record["index"]): record["normalized_prediction"] for record in records
        }
    ...
    def _prediction_result_from_generation(
        ...
        observed_labels = sorted(
            {str(record["normalized_gold"]) for record in records}
        ...
        predicted_texts = [str(record["normalized_prediction"]) for record in records]
        ...
        return PredictionResult(
            ...
            confidences=[
                float(record.get("generation_confidence", 0.0)) for record in records
            ],
            entropies=[
    ...
    def predict_for_augmentation(
        self, model: Trainer, split: str = "train"
    ) -> List[int]:
    ...
    def load_model(self, model_path) -> Trainer:
        ...
        model_dir = (
            model_path / "model" if (model_path / "model").exists() else model_path
        )
        ...
        return Trainer(
            ...
            args=args,
            tokenizer=tokenizer,
            data_collator=self._build_data_collator(tokenizer=tokenizer),
        )
