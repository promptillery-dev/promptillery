...
def _plot_regret_curve(
    ...
    """One panel per dataset: dollars of regret vs. (log) volume, per selector."""
    plt = _load_pyplot()
    by_dataset: dict[str, dict[str, list[tuple[float, float]]]] = defaultdict(
        lambda: defaultdict(list)
    )
    for row in rows:
        volume = _safe_float(row.get("volume"))
        regret = _safe_float(row.get("regret_usd"))
        if volume is None or regret is None:
            continue
...
def write_paper_figures(
    ...
    *,
    ...
    plot_specs: list[tuple[str, Iterable[Path]]] = [
        ...
        ("regret_curve.csv", [report_dir / "regret_curve.csv"]),
    ]
    ...
    regret_rows = _read_csv(report_dir / "regret_curve.csv")
    ...
    if regret_rows:
        created.extend(_plot_regret_curve(regret_rows, output_dir, fmt))
