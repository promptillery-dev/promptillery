...
_FINAL_NUMBER_PATTERN = re.compile(r"-?\d+(?:\.\d+)?")


def extract_final_number(text) -> str | None:
    """Return the last number in ``text`` (comma-stripped), or None.

    GSM8K's ``#### <answer>`` convention makes the final answer the last
    number in a solution; this is the single source of truth for the SLM
    trainer's ``answer_extraction: "number"`` mode, baseline_eval's
    generative task, augmentation screening, and the G4/G5 scripts.
    """
    matches = _FINAL_NUMBER_PATTERN.findall(str(text).replace(",", ""))
