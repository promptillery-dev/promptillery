...
# Keep in sync with promptillery.trainers.SFT_STUDENT_TYPES (not imported
# here to avoid a config<->trainers import cycle).
NUM_CLASSES_OPTIONAL_STUDENT_TYPES = {"slm", "causal_lm_sft"}
...
class DatasetConfig(BaseModel):
    ...
    num_classes: Optional[int] = Field(
        ...
        ge=2,
        description=(
            "Number of classes. Optional for slm students on label-free "
            "generative tasks; required for classifier students."
        ),
    )
...
class ExperimentConfig(BaseModel):
    ...
    def _require_num_classes_for_classifiers(self):
        if self.student_type in NUM_CLASSES_OPTIONAL_STUDENT_TYPES:
            return self
        entries = self.dataset_config
        if entries is None or isinstance(entries, str):
            return self
        if not isinstance(entries, list):
            entries = [entries]
        for entry in entries:
            if isinstance(entry, DatasetConfig) and entry.num_classes is None:
                raise ValueError(
                    f"dataset_config '{entry.name}' omits num_classes, which is "
                    f"required for student_type={self.student_type!r}"
                )
        return self
