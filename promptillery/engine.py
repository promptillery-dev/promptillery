...
from .trainers import SFT_STUDENT_TYPES, TrainerFactory
from .utils import (
...
def screen_votes(record_answer, solve_answers, agreement):
    """Self-consistency majority: the record's own answer counts as one vote."""
    if not record_answer:
        return False
    votes = [record_answer, *solve_answers]
    return sum(1 for vote in votes if vote == record_answer) >= agreement
...
class DistillationEngine:
    ...
    def _record_external_sft_token_usage(self) -> None:
        ...
        if (
            self.cfg.student_type not in SFT_STUDENT_TYPES
            or self._external_sft_usage_recorded
        ):
            return
    ...
    def _augmentation_response_format(self):
        ...
        if self.cfg.student_type in SFT_STUDENT_TYPES:
    ...
    def _build_augmented_sft_rows(
        ...
        *,
        ...
        # Optional deterministic wrap: the teacher emits bare problem text
        # (see the augmentation prompt), but seed/eval rows carry the full
        # solve scaffold from the paired materialize config's
        # student_prompt_template. Wrapping here — instead of asking the
        # teacher to reproduce the scaffold — keeps augmented and seed rows
        # byte-exact in prompt form with no LLM-reproduction drift. Screening
        # (in _augment, before this method runs) wraps bare problems itself
        # via SCREENING_SOLVE_PROMPT, so it needs no change and nothing here
        # double-wraps it.
        wrap_template = trainer_config.get("augmentation_student_prompt_template")
        wrap_jinja_template = (
            create_prompt_environment().from_string(wrap_template)
            if wrap_template
            else None
        )
        rows = []
        for index, record in enumerate(records):
            ...
            if not student_prompt or not teacher_response:
                continue
            if wrap_jinja_template is not None:
                student_prompt = wrap_jinja_template.render(question=student_prompt)
    ...
    def _screening_config(self) -> dict:
        config = (getattr(self.cfg, "trainer_config", None) or {}).get(
            "augmentation_screening"
        ) or {}
        return config if config.get("enabled") else {}

    async def _screen_augmented_records(self, records, *, cycle, teacher_model):
        """Self-consistency gate for synthetic SFT records (fail closed)."""
        screening = self._screening_config()
        if not screening or self.cfg.student_type not in SFT_STUDENT_TYPES:
            return records, {}
        k = int(screening.get("k", 3))
        agreement = int(screening.get("agreement", 2))
        temperature = float(screening.get("temperature", 0.7))
        kept, rejected, failed = [], 0, 0
        for index, record in enumerate(records):
            # Screen the same answer _build_augmented_sft_rows trains on:
            # gold_answer with teacher_response as the fallback.
            target = extract_final_number(
                str(
                    record.get("gold_answer")
                    or record.get("teacher_response")
                    or ""
                )
            )
            solve_answers = []
                ...
                for _ in range(k - 1):
                    completion_kwargs = {
                        ...
                        "messages": [
                            {
                                "role": "user",
                                "content": SCREENING_SOLVE_PROMPT.format(
                                    problem=record["student_prompt"]
                                ),
                            }
                        ],
                    ...
                    self.token_tracker.record_manual_usage(
                        extract_usage_from_response(result), OperationType.SCREENING
                    )
                    solve_answers.append(
                        extract_final_number(result["choices"][0]["message"]["content"])
                    )
            except Exception as e:
                # Fail closed: an unverifiable record must not enter training.
                logger.warning(
                    "Screening failed for cycle=%s record=%s: %s", cycle, index, e
                )
                failed += 1
                continue
            if screen_votes(target, solve_answers, agreement):
                kept.append(record)
            else:
                rejected += 1
        meta = {
        ...
        logger.info(
            "Self-consistency screen: kept %d/%d (rejected=%d, failed=%d)",
            len(kept), len(records), rejected, failed,
        )
        return kept, meta
    ...
    async def _augment(
        ...
        try:
            ...
            parsed_records, screening_meta = await self._screen_augmented_records(
                parsed_records, cycle=cycle, teacher_model=teacher_model
            )
            if screening_meta:
                attempt_metadata.update(screening_meta)
                if not parsed_records:
                    logger.warning("All augmented records rejected by screening")
                    self._record_teacher_attempt(
                        cycle=cycle,
                        status="screened_out",
                        predicted_cost=predicted_cost,
                        ...
                        decision_id=decision_id,
                        realized_cost=usage,
                        failure_type="all_records_screened_out",
                        metadata={**attempt_metadata, "records_added": 0},
                    )
                    attempt_logged = True
                    return {
                        "action_name": "augment_screened_out",
                        "predicted_cost": predicted_cost,
                        "metadata": {
                            "records_added": 0,
                            "attempt_id": attempt_id,
