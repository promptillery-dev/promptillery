...
class EvalResult:
    ...
    true_labels: List[Any] = field(default_factory=list)
    errors: int = 0
...
def build_generative_prompt(question, few_shot_examples=None):
    """Build the free-form solve prompt (zero-shot or with worked exemplars)."""
    parts = [GENERATIVE_SOLVE_INSTRUCTIONS, ""]
    if few_shot_examples:
        parts.append("Examples:")
        for example in few_shot_examples:
            parts.append(f"Problem: {example['question']}")
            parts.append(f"Solution: {example['answer']}")
            parts.append("")
...
async def solve_single(question, teacher, few_shot_examples=None, max_retries=3):
    """Solve one problem; returns (extracted final answer, error, token usage)."""
    prompt = build_generative_prompt(question, few_shot_examples)
    last_usage = None
    for attempt in range(max_retries):
        try:
            result = await acompletion(
            ...
            last_usage = _extract_usage_from_response(result)
            content = result["choices"][0]["message"]["content"]
            answer = extract_final_number(content)
            if answer is None:
                return None, f"No numeric answer in: {content[:80]!r}", last_usage
            return answer, None, last_usage
        except Exception as e:
            if attempt < max_retries - 1:
                await asyncio.sleep(0.5 * (attempt + 1))
            else:
                return None, str(e), last_usage
    return None, "Max retries exceeded", last_usage


async def evaluate_dataset_generative(
    dataset,
    teacher,
    mode="zero-shot",
    num_shots=5,
    ...
    """Free-form solve + exact-match on the final number (GSM8K protocol).

    Unlike the classification path, errors score as wrong (no random-label
    fallback) and few-shot exemplars are num_shots TOTAL, drawn from train.
    """
    eval_split = (
        "test" if "test" in dataset else "validation" if "validation" in dataset else "train"
    )
    eval_ds = dataset[eval_split]

    few_shot_examples = None
    few_shot_split = "train" if "train" in dataset else eval_split
    used_indices = set()
    if mode == "few-shot":
        rng = random.Random(seed)
        pool = range(len(dataset[few_shot_split]))
        used_indices = set(rng.sample(list(pool), min(num_shots, len(dataset[few_shot_split]))))
        few_shot_examples = [
            {
                "question": dataset[few_shot_split][i][question_column],
                "answer": dataset[few_shot_split][i][answer_column],
            }
    ...
    random.seed(seed)
    all_indices = list(range(len(eval_ds)))
    if mode == "few-shot" and eval_split == few_shot_split:
        all_indices = [i for i in all_indices if i not in used_indices]
    if max_samples and max_samples < len(all_indices):
        random.shuffle(all_indices)
        all_indices = all_indices[:max_samples]
    ...
    async def solve_with_semaphore(index):
        async with semaphore:
            pred, error, usage = await solve_single(
                eval_ds[index][question_column], teacher, few_shot_examples
            )
            return index, pred, error, usage

    results = await tqdm_asyncio.gather(
        *[solve_with_semaphore(i) for i in all_indices], desc=f"Solving ({mode})"
    )

    correct, errors = 0, 0
    predictions, golds = [], []
    for index, pred, error, usage in results:
        gold = extract_final_number(eval_ds[index][answer_column])
        predictions.append(pred)
        golds.append(gold)
    ...
    exact_match = correct / len(all_indices) if all_indices else 0.0
    return EvalResult(
        dataset="",
        dataset_config="",
        mode=mode,
...
async def run_baseline_evaluation(
    ...
    Args:
        ...
        num_shots: Number of examples per class for few-shot (classification) or
            TOTAL exemplars (generative)
        max_samples: Maximum samples per evaluation
        output_dir: Output directory for results
        seed: Random seed
        concurrency: Max concurrent API calls
        task: "classification" (default) or "generative"
        data_files: generative task only; list of "SPLIT=PATH" JSONL specs
        question_column: generative task only; name of the question column
        answer_column: generative task only; name of the answer column
    ...
    if task == "generative":
        ...
        if not data_files:
            raise ValueError("--task generative requires --data-files SPLIT=PATH ...")
        files = dict(spec.split("=", 1) for spec in data_files)
        dataset = hf_load_dataset("json", data_files=files)
        for mode in modes:
            result = await evaluate_dataset_generative(
                dataset,
                teacher=teacher,
                mode=mode,
                num_shots=num_shots,
            ...
            result.dataset = dataset_name or "generative"
            result.dataset_config = "generative"
            results.append(result)
        # reuse the existing save/summary tail (print_summary_table/save_summary_csv)
        output_path = Path(output_dir)
        output_path.mkdir(parents=True, exist_ok=True)
        print_summary_table(results)
        save_summary_csv(results, output_path / "baseline_summary.csv")
        return results
...
def print_summary_table(results: List[EvalResult]) -> None:
    ...
    for r in results:
        # Classification results always carry "accuracy"/"f1"; generative results
        # (metrics={"exact_match": ...}) fall back to exact_match with no F1.
        primary_metric = r.metrics.get("accuracy", r.metrics.get("exact_match"))
        f1_metric = r.metrics.get("f1")
        table.add_row(
            r.dataset,
            r.dataset_config,
            r.mode,
            str(r.num_shots) if r.mode == "few-shot" else "-",
            f"{primary_metric:.4f}" if primary_metric is not None else "-",
            f"{f1_metric:.4f}" if f1_metric is not None else "-",
            str(r.num_samples),
...
def main():
    ...
    parser.add_argument(
        ...
        help="generative = free-form solve scored by final-number exact match; "
        "--num-shots then means TOTAL exemplars, not per-class",
    )
    parser.add_argument(
        "--data-files",
        nargs="+",
        metavar="SPLIT=PATH",
        help="generative task: JSONL splits, e.g. train=out/g4/gsm8k/train.jsonl "
        "test=out/g4/gsm8k/test.jsonl",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Few-shot exemplar count: per class for --task classification, "
        "TOTAL exemplars drawn from the train split for --task generative "
        "(default: 2)",
    )
    parser.add_argument(
        "--max-samples",
