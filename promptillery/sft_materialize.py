...
from .engine import (
    ...
    PREFLIGHT_PROVIDER_TOKEN_SAFETY_RATIO,
    ensure_class_label,
...
async def materialize_sft_records(
    *,
    ...
    ``token_budget`` overrides ``config.token_budget`` for this materialization
    only. The teacher test-label file used for fidelity is a one-time cost that
    is separate from the run's AL-loop budget, so it can be labeled over the
    full split without inflating the protocol budget the run reports.
    """
    if token_budget is not None:
        config = config.model_copy(update={"token_budget": token_budget})
            ...
            with temp_path.open("w", encoding="utf-8") as f:
                for index, row in enumerate(source):
                    ...
                    if mode == "teacher":
                        ...
                        raw_predicted_total = _estimate_input_tokens(
                            config.teacher, messages
                        ) + int(config.teacher_max_output_tokens or 0)
                        # The input-token count is only an estimate (litellm's
                        # token_counter or a chars/3 fallback), so the realized
                        # provider total routinely lands a few tokens above it.
                        # With a tiny output cap (materialization uses 16) that
                        # noise alone trips the realized>predicted guard on row 0.
                        # Absorb it with the same safety margin the AL loop uses
                        # (engine._estimate_teacher_call_tokens); a genuine output
                        # runaway still overshoots the margin and hard-fails.
                        predicted_total = raw_predicted_total + max(
                            PREFLIGHT_PROVIDER_TOKEN_SAFETY_MIN,
                            math.ceil(
                                raw_predicted_total
                                * PREFLIGHT_PROVIDER_TOKEN_SAFETY_RATIO
